...
export default async function handler(req, res) {
  ...
  try {
    ...
    const customerId = session && session.customer_id
    ...
    const rpcRes = await fetch(
      SUPABASE_URL + "/rest/v1/rpc/vip_get_vip_ranking",
      {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    if (!rpcRes.ok) {
      ...
      return res.status(500).json({
        ok: false,
        error: "Error obteniendo ranking VIP",
        detail: txt
      });
    }

    const ranking = await rpcRes.json();
